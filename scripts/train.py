...
# --- FUNGSI MENCARI BATCH SIZE OPTIMAL ---
def find_optimal_batch_size(model_config, training_config, dataset_path, element_map_path,
                            start_bs: int = 8, max_bs: int = 512, step_strategy: str = "progressive"):
    """
    Mencari batch size terbesar yang masih muat memori (GPU/CPU) dengan satu forward + backward pass cepat.
    Strategi:
      - Mulai dari start_bs.
      - Jika 'progressive': gandakan (8 -> 16 -> 32 ...) sampai gagal, lalu lakukan pencarian biner rentang terakhir sukses-gagal.
      - Jika 'linear': tambah secara linear (misal +8) sampai gagal.
    Kriteria gagal: RuntimeError mengandung 'out of memory' atau sistem memori > 95%.
    """
    print("\n--- MODE PENCARIAN BATCH SIZE OPTIMAL ---")
    ...
    # Dataset kecil untuk uji (gunakan subset train)
    test_dataset = HDF5SpectrumDataset(file_path=dataset_path, split='train', max_samples= min(1024,  len(HDF5SpectrumDataset(dataset_path,'train')) if hasattr(HDF5SpectrumDataset(dataset_path,'train'),'__len__') else 1024))

    def can_run(bs: int) -> bool:
        try:
            torch.cuda.empty_cache() if torch.cuda.is_available() else None
            loader = DataLoader(test_dataset, batch_size=bs, shuffle=True, num_workers=0)
            model = InformerModel(**model_config).to(device)
            ...
            with autocast(device_type=device if device!='cpu' else 'cpu', dtype=torch.float16 if device!='cpu' else torch.bfloat16):
                outputs = model(data)
            ...
            if ram.percent > 95:
                mem_ok = False
            del model, optimizer, criterion, loader, data, labels, outputs, loss
            torch.cuda.empty_cache() if torch.cuda.is_available() else None
            return mem_ok
    ...
    if step_strategy == 'progressive':
        # Ekspansi eksponensial
        while current <= max_bs:
            print(f"Menguji batch size {current} ...", end=' ')
            ok = can_run(current)
            tried.append((current, ok))
            print("OK" if ok else "GAGAL")
            if ok:
                best = current
                current *= 2
            else:
                failed_upper = current
                break
        # Binary search antara best dan failed_upper
        if best and failed_upper and failed_upper - best > 4:
            low, high = best + 1, failed_upper - 1
            while low <= high:
                mid = (low + high) // 2
                print(f"[Binary] Uji {mid} ...", end=' ')
                ok = can_run(mid)
                tried.append((mid, ok))
                print("OK" if ok else "GAGAL")
                if ok:
    ...
    print("\nRingkasan Percobaan Batch Size:")
    for bs, ok in sorted(tried, key=lambda x: x[0]):
        print(f"  - {bs}: {'OK' if ok else 'FAIL'}")
    if best is None:
        print("Tidak ada batch size yang berhasil. Coba kurangi dimensi model atau panjang sequence.")
    else:
        print(f"Batch size optimal (perkiraan aman): {best}")
    print("Selesai mencari batch size. Keluar.")
    return best
...
def main(args):
    ...
    CONFIG['training']['model_save_path'] = os.path.join(args.model_dir, 'informer_multilabel_model-4096-2.pth')
    CONFIG['training']['class_weight_path'] = os.path.join(args.model_dir, 'class_weights_multilabel-4096-2.pth')
    CONFIG['training']['progress_save_path'] = os.path.join(args.model_dir, 'training_progress-4096-2.json')
